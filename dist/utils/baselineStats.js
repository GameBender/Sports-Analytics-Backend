...
class BaselineStats {
     ...
     */
    calculateEV(modelProbability, americanOdds) {
     ...
     */
    calculateTeamQuality(homeTeam, awayTeam) {
     ...
     */
    calculateFormImpact(homeTeam, awayTeam) {
     ...
     */
    calculateInjuryImpact(homeInjuries, awayInjuries, league) {
     ...
     */
    getWeatherImpact(league, weather) {
        // Default no impact
    ...
    assessDataQuality(game) {
        ...
        if (game.weather && this.isOutdoorSport(game.league))
            quality += 5;
        if (game.homeTeam?.injuries && game.awayTeam?.injuries)
            quality += 5;
        if (game.homeTeam?.recentForm && game.awayTeam?.recentForm)
            quality += 10;
        return Math.min(100, quality);
    }
}
exports.BaselineStats = BaselineStats;
